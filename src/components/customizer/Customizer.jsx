...
export default function Customizer() {
  ...
  const handleGeneration = (inputValues, inputData, placeholder) => {
    const variants = [];
    ...
    afwerkingen.forEach((afwerking, afwerkingIndex) => {
      Object.keys(dikteObj).forEach((key) => {
        ...
        if (
          !splitSKU[
            afwerkingIndex * Object.keys(dikteObj).length + parseInt(key)
          ]
        ) {
          return;
        }
    ...
    inputData.description = updatedDescription;
    return inputData;
  };
  // customData.desc = "hoi";
  const changeCurrentDescription = useCallback(() => {
    switch (selectedOption) {
      case "Dekton":
        setUpdatedDescription(`<h1><strong>Dekton ${inputValues.naam} keramiek keukenbladen</strong></h1>
        <a href="https://stonecenter-shop.nl/">StoneCenter</a><strong> fabriceert</strong> naast natuursteen- en composiet- ook <strong>keramische keukenbladen</strong> waaronder het Dekton  keramiek keukenblad. Dekton is een begrip op het gebied van keramiek keukenbladen en heeft zich gespecialiseerd tot een van de beste keramisch leveranciers die er zijn. Met name kwaliteit en hun brede assortiment is het grote voordeel van Dekton.
        
        Naast de toepassing voor Dekton ${inputValues.naam} keramiek keukenbladen, kan het materiaal ook voor velen andere toepassingen gebruikt worden, denk aan: gevels, wanden, vloeren wanden en alle andere bedenkbare toepassingen.
        <h2><strong>Wat is Dekton ${inputValues.naam} keramiek?</strong></h2>
        Een Dekton ${inputValues.naam} keramiek keukenbladen zijn gemaakt van een mix van hoogwaardige grondstoffen om de beste eigenschappen aan het werkblad oppervlak mee te geven. Dekton keramiek is veelzijdig en kan voor verschillende bouwprojecten uw beste keuze zijn. Dekton is beschikbaar in verschillende diktes van 8 mm tot wel 30 mm met een afmeting van 320 x 144 cm, zo is het relatief licht van gewicht waardoor het uitermate geschikt is voor keukens en badkamers ook voor gevels en wanden. Ook voor intensief gebruikte vloeren is Dekton het ideale materiaal.
        <h3><strong>Voordelen van een Dekton ${inputValues.naam} keramiek keukenblad</strong></h3>
